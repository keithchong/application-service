	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want.GitSource != nil {
				...
				//assert.True(t, tt.want.GitSource.Revision == mappedComponent.GitSource.Revision, "Expected GitSource Revision: %s, is different than actual: %s", tt.want.GitSource.Revision, mappedComponent.GitSource.Revision)
				//assert.True(t, tt.want.GitSource.Context == mappedComponent.GitSource.Context, "Expected GitSource Context: %s, is different than actual: %s", tt.want.GitSource.Context, mappedComponent.GitSource.Context)
				//assert.True(t, tt.want.GitSource.DevfileURL == mappedComponent.GitSource.DevfileURL, "Expected GitSource DevfileURL: %s, is different than actual: %s", tt.want.GitSource.DevfileURL, mappedComponent.GitSource.DevfileURL)
				//assert.True(t, tt.want.GitSource.DockerfileURL == mappedComponent.GitSource.DockerfileURL, "Expected GitSource DockerfileURL: %s, is different than actual: %s", tt.want.GitSource.DockerfileURL, mappedComponent.GitSource.DockerfileURL)
			}
		})
	}
}
